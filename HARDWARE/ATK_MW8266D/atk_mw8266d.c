...
void atk_mw8266d_hw_init(void)
{
    ...
    // 启用GPIO时钟
    RCC_APB2PeriphClockCmd(ATK_MW8266D_RST_GPIO_PORT_CLK, ENABLE);

    // 初始化GPIO结构体
    GPIO_InitStructure.GPIO_Pin = ATK_MW8266D_RST_GPIO_PIN;         // 设置引脚
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;               // 推挽输出模式
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;              // 设置速度

    // 配置GPIO端口
    GPIO_Init(ATK_MW8266D_RST_GPIO_PORT, &GPIO_InitStructure);

    // 拉高引脚（默认为高电平）
    GPIO_SetBits(ATK_MW8266D_RST_GPIO_PORT, ATK_MW8266D_RST_GPIO_PIN);
}
