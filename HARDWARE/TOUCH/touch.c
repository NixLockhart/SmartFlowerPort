...
void tp_save_adjust_data(void)
{
    ...
    AT24CXX_WriteOneByte(TP_SAVE_ADDR_BASE + 12, 0X0A);   /* 保存校准值 */
}
...
uint8_t tp_get_adjust_data(void)
{
    ...
    temp = AT24CXX_ReadOneByte(TP_SAVE_ADDR_BASE + 12);   /* 读取校准状态标记 */
...
uint8_t tp_init(void)
{
    ...
    {
        if (GT9147_Init() == 0)             /* 初始化GT系列触摸屏成功,即当前3.5寸屏为电容触摸屏 */
        {
